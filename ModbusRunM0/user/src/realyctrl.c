...
void HWRealyInit(void)
{
  ...
  --ref 比特电子 GTLCS207-ER4GS T1 V1 PCB and SHC */
	  ...
	  GPIO_SetMode(P0, BIT0|BIT3, GPIO_PMD_OUTPUT); //relay
	  GPIO_SetMode(P1, BIT3, GPIO_PMD_OUTPUT); //relay
	  GPIO_SetMode(P2, BIT2, GPIO_PMD_OUTPUT); //relay
	  P00=0;
	  P03=0;
	  P13=0;
	  P42=0;
...
uint8_t GetHwAddrIint(void)
{
 ...
 GPIO_SetMode(P4, BIT4, GPIO_PMD_INPUT);
	...
	addr[3]<<=0;//get bit of ADDR0
	 ...
	 GPIO_SetMode(P0, BIT4|BIT5|BIT6, GPIO_PMD_INPUT);
	 GPIO_SetMode(P4, BIT1, GPIO_PMD_INPUT);
	addr[0]=P06;//get bit of ADDR3
	addr[1]=P05;//get bit of ADDR2
	addr[2]=P04;//get bit of ADDR1
	addr[3]=P41;//get bit of ADDR0
